    ...
    async fn new(window: Arc<Window>) -> anyhow::Result<Self> {
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                ...
                buffers: &[silver_broccoli::Vertex::desc()],
        ...
        let vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            ...
            contents: bytemuck::cast_slice(silver_broccoli::VERTICES),
            ...
            WindowEvent::RedrawRequested => {
                ...
                match state.render() {
                    ...
                    Err(error) => match error {
                        RenderError::SurfaceError(wgpu::SurfaceError::Lost | wgpu::SurfaceError::Outdated) => {
                            let size = state.window.inner_size();
                            state.resize(size.width, size.height);
                        }
                        error => log::error!("Unable to render {error:?}"),
                    }
                }
            },
            _ => {},
